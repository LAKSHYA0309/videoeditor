...
export default function SignIn() {
  ...
  const handleGuestLogin = async () => {
    ...
    try {
      try {
        await signIn('guest@editro.com', 'guestpassword')
      } catch (err) {
        // If guest user doesn't exist yet, sign them up
        await signUp('guest@editro.com', 'guestpassword', 'Guest User')
      }
      navigate('/dashboard')
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-[#fafafa] relative overflow-hidden font-sans selection:bg-blue-500/20 selection:text-white bg-grid-pattern flex items-center justify-center px-6">
      {/* Glow effect */}
      <div className="absolute top-[20%] right-[30%] w-[40%] h-[40%] rounded-full bg-blue-500/[0.03] blur-[130px] pointer-events-none" />
      ...
      >
        ...
        <div className="text-left">
          ...
          <p className="text-[14px] text-neutral-400 font-[400] leading-relaxed">
            Enter your credentials to access your editor workspace.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="space-y-3 pt-2">
            ...
            <button
              ...
              disabled={loading}
              className="w-full rounded-lg border border-white/[0.08] hover:bg-white/[0.04] text-[#fafafa] py-2.5 text-[15px] font-[600] disabled:opacity-50 active:scale-98 transition-all flex items-center justify-center font-sans"
            >
              Continue as Guest
            </button>
          </div>
        ...
        <div className="text-center text-[13px] border-t border-white/[0.04] pt-4">
          <p className="text-neutral-400">
            ...
            <Link to="/sign-up" className="text-blue-400 hover:text-blue-300 hover:underline transition-colors font-medium">
              Create account
            </Link>
          </p>
        </div>
